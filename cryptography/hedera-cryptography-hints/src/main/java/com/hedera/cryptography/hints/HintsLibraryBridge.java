...
public class HintsLibraryBridge {
     ...
     * @return the 128-byte EIP-2537 encoding, or null on error
     */
    public byte[] decompressG1ToEip2537Blst(final byte[] compressedPoint) {
        if (compressedPoint == null || compressedPoint.length != COMPRESSED_G1_LENGTH_BYTES) {
            return null;
        }
        return decompressG1ToEip2537BlstImpl(compressedPoint);
    }

    private native byte[] decompressG1ToEip2537BlstImpl(final byte[] compressedPoint);
     ...
     * @return the 256-byte EIP-2537 encoding, or null on error
     */
    public byte[] decompressG2ToEip2537Blst(final byte[] compressedPoint) {
        if (compressedPoint == null || compressedPoint.length != COMPRESSED_G2_LENGTH_BYTES) {
            return null;
        }
        return decompressG2ToEip2537BlstImpl(compressedPoint);
    }

    private native byte[] decompressG2ToEip2537BlstImpl(final byte[] compressedPoint);
